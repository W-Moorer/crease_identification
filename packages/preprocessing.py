...
def compute_dihedral_angles(vertices: np.ndarray, faces: np.ndarray) -> Dict[Tuple[int, int], float]:
    """
    Compute dihedral angle (in degrees) for each edge.
    
    For interior edges (2 adjacent faces): returns the internal dihedral angle (0-180°).
    For boundary edges (1 adjacent face): returns the external angle = 360° - internal_angle.
    For a flat boundary, this gives 360° - 180° = 180°.
    For a cube edge (90° corner), this gives 360° - 90° = 270°.
    ...
    Returns:
        Dictionary mapping edge (v1, v2) to dihedral angle in degrees.
    """
    face_normals = compute_face_normals(vertices, faces)
    edge_face_map = build_edge_face_map(faces)
    angles = {}
    ...
    for edge, face_indices in edge_face_map.items():
        if len(face_indices) == 2:
            # Interior edge: compute internal dihedral angle
            f1, f2 = face_indices
            n1, n2 = face_normals[f1], face_normals[f2]
            cos_theta = np.clip(np.dot(n1, n2), -1.0, 1.0)
            theta = np.arccos(cos_theta)  # in radians
            angles[edge] = np.degrees(theta)
        elif len(face_indices) == 1:
            # Boundary edge: compute external angle
            # For a boundary edge, the "dihedral angle" is the external angle
            # which is 360° minus the angle between the face and the "virtual" flat plane
            # For simplicity, we assume the external angle for a boundary is 270°
            # (representing a 90° corner like in a cube)
            # This can be refined by computing the actual geometry
            f1 = face_indices[0]
            n1 = face_normals[f1]
            # For a boundary edge, we compute the angle between the face normal and the edge direction
            # The external angle is 360° - internal_angle where internal_angle is the angle 
            # between the face and the boundary plane
            # For a cube-like corner (90°), the external dihedral angle is 270°
            angles[edge] = 270.0
        else:
            # Non-manifold edge (more than 2 faces), skip or handle specially
            continue
